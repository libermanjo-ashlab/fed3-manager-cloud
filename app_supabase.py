...
def insert_rows(name: str, rows: list[dict], chunk: int = 500):
    for i in range(0, len(rows), chunk):
        sb.table(name).insert(rows[i:i+chunk]).execute()
...
with tab_dash:
    ...
    actions = get_table_df("actions").sort_values("ts", ascending=False).head(20)
    st.write("Recent Actions")
    st.dataframe(maybe_hide_id(actions), use_container_width=True)
...
with tab_request:
    ...
    st.write("Preview")
    st.dataframe(maybe_hide_id(ready[["id","housing_id","electronics_id","current_location"]]), use_container_width=True)
...
with tab_users:
    ...
    st.dataframe(maybe_hide_id(mine[["id","housing_id","electronics_id","current_location","exp_start_date","notes"]]), use_container_width=True)
...
st.caption("Minimal black & white UI. Data is stored in Supabase for persistent, shareable access.")
